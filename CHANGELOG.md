...
- **`nyp_apartments.listing_star`:** nullable tinyint (1 Peter, 2 Kerv, 3 both); `migrate.js` ALTER; `GET/POST/PUT` apartments + `apartmentRepository` + `apartmentSavePayload` + admin `readApartmentForm` preserves star from loaded row; **`assets/js/listingStar.js`** (`NyhomeListingStar`) for SVG tier UI — wire script on pages when enabling star controls (not on shortlist shell yet).
