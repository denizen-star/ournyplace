...
const { boundsForInstantInTz, ymdInTz, startMsOfYmdInTz } = require('./etDayBounds');
...
@media only screen and (max-width: 600px) {
  ...
  .nyhome-cal-2col td.nyhome-cal-cell {
    display: block !important;
    width: 100% !important;
    box-sizing: border-box !important;
    border-bottom: 1px solid ${C.tableBorder} !important;
  ...
  .nyhome-cal-2col tr { display: block !important; }
  .nyhome-cal-2col td.nyhome-cal-cell:last-child { border-bottom: none !important; }
}
</style>`;
...
function digestCalendarKindLabel(kind) {
  if (kind === 'tour') return 'Tour';
  if (kind === 'deadline') return 'App deadline';
  if (kind === 'movein') return 'Move-in';
  return 'Event';
}

function formatDigestEventTime(ms, kind, timeZone) {
  if (kind === 'movein') return 'All day';
  if (ms == null || Number.isNaN(ms)) return '—';
...
function formatDigestDayHeadingFromYmd(ymd, timeZone) {
  if (!ymd) return '';
  const ms = startMsOfYmdInTz(ymd, timeZone);
  return new Intl.DateTimeFormat('en-US', {
    weekday: 'long',
    month: 'short',
    day: 'numeric',
...
function collectDigestCalendarEvents(apartments, timeZone) {
  const out = [];
  (apartments || []).forEach((apt) => {
    if (TERMINAL_EXCLUDE.has(normalizeStatus(apt.status))) return;
    const visitMs = apt.next_visit && apt.next_visit.visit_at ? parseTime(apt.next_visit.visit_at) : null;
    if (visitMs != null) {
      out.push({ kind: 'tour', sortTs: visitMs, ymd: calendarDateInTz(visitMs, timeZone), apt });
    }
    const deadlineMs = apt.application && apt.application.deadline_at ? parseTime(apt.application.deadline_at) : null;
    if (deadlineMs != null) {
      out.push({ kind: 'deadline', sortTs: deadlineMs, ymd: calendarDateInTz(deadlineMs, timeZone), apt });
    }
    const moveRaw = apt.move_in_date != null ? String(apt.move_in_date).trim() : '';
    if (moveRaw) {
      const moveMs = parseMoveInNoonMs(moveRaw);
      if (moveMs != null && !Number.isNaN(moveMs)) {
        out.push({ kind: 'movein', sortTs: moveMs, ymd: calendarDateInTz(moveMs, timeZone), apt });
      }
    }
  });
  return out;
...
function buildCalendarTwoDayHtml(apartments, digestBounds, nowMs, base, timeZone) {
  const bounds = digestBounds || boundsForInstantInTz(nowMs, timeZone);
  const todayYmd = bounds.ymdET || calendarDateInTz(nowMs, timeZone);
  const tomorrowYmd = ymdInTz(bounds.endExclusiveMs, timeZone);

  const all = collectDigestCalendarEvents(apartments, timeZone);
  const pick = (ymd) => all.filter((e) => e.ymd === ymd).sort((a, b) => a.sortTs - b.sortTs);

  function renderDay(ymd) {
    const list = pick(ymd);
    if (list.length === 0) {
      return `<p style="margin:0;font-size:14px;color:${C.muted};line-height:1.55;">Nothing scheduled.</p>`;
    }
    const items = list
      .map((e) => {
        const title = escapeHtml(e.apt.title || 'Listing');
        const kind = escapeHtml(digestCalendarKindLabel(e.kind));
        const when = escapeHtml(formatDigestEventTime(e.sortTs, e.kind, timeZone));
        const link =
          e.apt.id != null && base
            ? ` <a href="${escapeHtml(detailHref(base, e.apt.id))}" style="color:${C.link};font-weight:600;font-size:13px;">Details</a>`
            : '';
        return `<li style="margin:0 0 12px;line-height:1.45;"><span style="font-size:12px;font-weight:700;color:${C.accentDeep};text-transform:uppercase;letter-spacing:0.05em;">${kind}</span><br><strong style="color:${C.ink};">${title}</strong><br><span style="font-size:14px;color:${C.muted};">${when}</span>${link}</li>`;
      })
      .join('');
    return `<ul style="margin:0;padding-left:18px;list-style:disc;color:${C.ink};">${items}</ul>`;
  }

  const colShared = `vertical-align:top;width:50%;padding:16px 14px;background:${C.cardBg};`;
  return `${digestH2(digestBadge('Ca', C.badgeCal, C.badgeCalFg), 'Today & tomorrow', 4)}
<p style="margin:0 0 12px;font-size:15px;color:${C.muted};line-height:1.55;">Tours, application deadlines, and move-in dates in <strong>Eastern</strong>—same signals as Next actions.</p>
<table class="nyhome-cal-2col" role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 24px;border-collapse:collapse;border:1px solid ${C.tableBorder};border-radius:12px;overflow:hidden;">
<tr>
<td class="nyhome-cal-cell" style="${colShared}border-right:1px solid ${C.tableBorder};">
<p style="margin:0 0 10px;font-size:13px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:${C.muted};">Today</p>
<p style="margin:0 0 12px;font-size:17px;font-family:Georgia,'Times New Roman',serif;font-weight:600;color:${C.ink};">${escapeHtml(formatDigestDayHeadingFromYmd(todayYmd, timeZone))}</p>
${renderDay(todayYmd)}
</td>
<td class="nyhome-cal-cell" style="${colShared}">
<p style="margin:0 0 10px;font-size:13px;font-weight:700;letter-spacing:0.06em;text-transform:uppercase;color:${C.muted};">Tomorrow</p>
<p style="margin:0 0 12px;font-size:17px;font-family:Georgia,'Times New Roman',serif;font-weight:600;color:${C.ink};">${escapeHtml(formatDigestDayHeadingFromYmd(tomorrowYmd, timeZone))}</p>
${renderDay(tomorrowYmd)}
</td>
</tr>
</table>`;
}
...
${signedWins
  ...
  const boundsForCal = digestBounds || boundsForInstantInTz(now, DIGEST_TZ);
  const calendarBlock = buildCalendarTwoDayHtml(apartments, boundsForCal, now, base, DIGEST_TZ);
...
${calendarBlock}
${digestH2(digestBadge('Td', C.badgeToday, C.badgeTodayFg), 'Today', 28)}
...
module.exports = {
  ...
  TERMINAL_EXCLUDE,
  TOP_POOL_EXCLUDE,
  // Pure utility functions shared with adminAnalytics
