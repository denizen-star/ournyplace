...
/**
 * One apartment in the same shape as `getApartmentPayload().apartments[i]`, plus `criteria` row list.
 * Used by listing-added email after POST create.
 */
async function getApartmentById(rawId) {
  ...
  const [{ rows: criteria }, images, ratings, visits, applications] = await Promise.all([
    execute(`SELECT * FROM nyp_criteria WHERE active = TRUE ORDER BY sort_order ASC, id ASC`),
    ...
    id = await insert(
      `INSERT INTO nyp_apartments
         (title, address, apt_number, neighborhood, rent_cents, net_effective_cents,
          ...
          listing_url, source_url, import_status, status, notes, listing_star)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      params
    );
    await tryInsertListingEvent({
