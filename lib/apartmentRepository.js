...
const { criteriaForPartnerScoring } = require('./criteriaCompact');
const { getCompactVoting } = require('./appSettingsRepository');
...
async function getApartmentPayload() {
  const [compactVoting, { rows: apartments }, { rows: criteria }, { rows: neighborhoods }] = await Promise.all([
    getCompactVoting(),
    execute(
  ...
  if (apartments.length === 0) {
    return { apartments: [], criteria, neighborhoods, compactVoting };
  }
  ...
  return {
    ...
    apartments: apartments.map((apartment) => {
      ...
      return {
        ...
        scores: calculateScores(criteria, apartmentRatings, compactVoting),
...
async function getApartmentById(rawId) {
  ...
  const [compactVoting, { rows: criteria }, images, ratings, visits, applications, { rows: neighborhoods }] = await Promise.all([
    getCompactVoting(),
    execute(`SELECT * FROM nyp_criteria WHERE active = TRUE ORDER BY sort_order ASC, id ASC`),
...
function calculateScores(criteria, ratings, compactVoting) {
  const result = {};
  PARTNERS.forEach((partner) => {
    let weighted = 0;
    let includedWeight = 0;
    const critList = criteriaForPartnerScoring(criteria, partner, Boolean(compactVoting));
    critList.forEach((criterion) => {
