...
const { buildRejectionEmailHtml } = require('./rejectionEmail');
...
async function sendListingAddedEmail(apartmentId) {
  ...
  const { html, subject } = buildListingAddedEmailHtml({
    ...
    publicBaseUrl,
    compactVoting: Boolean(data.compactVoting),
  });
...
/**
 * Same layout as listing-added email when scoring is complete (`isBothPartnersVotingComplete`:
 * either every active criterion or, when compact voting is on, every primary row per partner).
 * @param {boolean} options.manual - from UI button: send only if complete (no DB flag on success path).
...
async function sendListingScoresCompleteEmail(apartmentId, options) {
  ...
  const compactVoting = Boolean(data.compactVoting);
  if (!isBothPartnersVotingComplete(criteria, apartment.ratings, compactVoting)) {
    return { skipped: true, reason: 'incomplete', sent: false };
  }
...
/**
 * Notify NYHOME_EMAIL_TO when a listing transitions into `rejected` (shortlist, details, admin).
 * Does not throw: logs and returns `{ sent, skipped, error? }`.
 */
async function sendRejectionEmail(apartmentId) {
  const cfg = getSmtpConfig();
  if (!smtpReady(cfg)) {
    console.warn('[rejection-email] skipped: SMTP or NYHOME_EMAIL_TO not configured');
    return { skipped: true };
  }

  const data = await getApartmentById(apartmentId);
  if (!data || !data.apartment) {
    console.error('[rejection-email] apartment not found', apartmentId);
    return { skipped: true };
  }

  const publicBaseUrl = normalizeBaseUrl(process.env.NYHOME_PUBLIC_URL) || '';
  const { html, subject } = buildRejectionEmailHtml({
    apartment: data.apartment,
    publicBaseUrl,
  });
  const transport = createTransport(cfg);
  ...
  try {
    ...
    console.error('[rejection-email] sendMail', e);
    return { skipped: false, sent: false, error: e.message };
  }
