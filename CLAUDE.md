...
- **`lib/pipelineDigest.js`** — HTML **pipeline digest** (needs attention, top 10 sort: star → status → combined avg, closed wins); **`netlify/functions/pipeline-digest-email.js`** — `POST` sends via SMTP (**`nodemailer`**). Env: `NYHOME_SMTP_*`, `NYHOME_EMAIL_FROM`, `NYHOME_EMAIL_TO` (comma-separated), optional `NYHOME_PUBLIC_URL`. Shortlist **Email digest** + Filters drawer button; Admin **Settings** tab saves **public site URL** in `localStorage` (`nyhomePublicBaseUrl`) for `/details` links. Heuristics (tunable in `DEFAULT_ATTENTION`): app deadline ≤7d or overdue; tour ≤48h without visit notes; `tour_scheduled` with visit >24h ago; `evaluating` unchanged >10d; finalist+ with no combined score.
